...
Common.init_build_directory = async (platform, template, initialized, templatedir, builddir, config_dict=Common.PLAT_CONFIG)  => {
	...
	if(!initialized){
	  ...
	  try{
	    ...
	    cap = ['cp', '-r', template_path, templatedir].join(' ')
            console.log('hello')
	    execSh(cap , {cwd: builddir}, function(err){
		if(err){ 
		  //console.log(err)		
		  console.log('failed to copy on:', err);	
		}
		else{
		  console.log('executed copy on:', template_path);	
		}
	    });
	  }
	  catch(exitCode){
